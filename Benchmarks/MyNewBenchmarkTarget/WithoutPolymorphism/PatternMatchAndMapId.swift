...
func patternMatchAndMapId() -> Int {
  ...
  for _ in 0..<10_000_00 {
    ...
    let nodeHeaderInfo = p.syntaxInfo[list[Int(num) % list.count]]

    switch nodeHeaderInfo.type {
    case .a:
      sum += p.differentStores.a[Int(nodeHeaderInfo.privateId)].v
    case .b:
      sum += p.differentStores.b[Int(nodeHeaderInfo.privateId)].v
    case .c:
      let e = p.differentStores.c[Int(nodeHeaderInfo.privateId)]
